...
try {
    switch ($action) {
        ...
        case 'send':
            ...
            // Check if recipient is a resident
            $recipient_query = "SELECT r.resident_id, r.first_name, r.last_name, r.family_member_id, u.user_id, u.role
                              FROM residents r
                              LEFT JOIN users u ON r.resident_id = u.user_id
                              WHERE r.resident_id = $1";
            $recipient_result = pg_query_params($db_connection, $recipient_query, array($_POST['recipient_id']));
            $recipient = pg_fetch_assoc($recipient_result);
            ...
            if ($recipient && $recipient['family_member_id']) {
                $recipient_id = $recipient['family_member_id'];
            }
