...
try {
    // Debug log
    error_log("Processing message from staff " . $_SESSION['staff_id']);
    ...
    // Debug log
    error_log("Staff user_id: " . $staff['user_id']);
    ...
    // Debug log
    error_log("Message data - recipient_id: $recipient_id, content: $content, priority: $priority, category: $category");

    // Verify recipient is an admin
    $recipient_query = "SELECT user_id FROM users WHERE user_id = $1 AND role = 'admin'";
    $recipient_result = pg_query_params($db_connection, $recipient_query, array($recipient_id));
    ...
    if (pg_num_rows($recipient_result) === 0) {
        error_log("Invalid recipient - user_id: $recipient_id is not an admin");
